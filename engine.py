...
try:
    ...
    print("[x] Error: selenium is not installed. Please install it using 'pip install selenium'")
    sys.exit(1)
...
class Bot:
    ...
    def start(self):
        ...
        service = self._get_service_key()
        if service:
            self._start_service(service, self.video_url)
        else:
            print("[!] Service unavailable or not found.")

    def _print_banner(self):
        print("+--------------------------------------------------------+")
        print("|                                                        |")
        print("|   TikTok Bot Engine                                    |")
        print("|                                                        |")
        print("+--------------------------------------------------------+")
        print("\n")
    ...
    def _solve_captcha(self):
        try:
            self._wait_for_element(By.TAG_NAME, "input", timeout=5)
            print("[~] Captcha detected. (Note: Cannot be solved automatically in headless mode)")
    ...
    def _wait_for_element(self, by, value, timeout=10):
        for _ in range(timeout):
            try:
                ...
                sleep(1)
        raise NoSuchElementException(f"Could not find element {value}")


if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python engine.py <service_type> <video_url>")
        sys.exit(1)
